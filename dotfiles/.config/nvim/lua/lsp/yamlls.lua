vim.api.nvim_create_autocmd("FileType", {
	...
	callback = function(ev)
		...
		local function apply_schema_to_buf(bufnr, schema_id)
			...
			if not client then
				vim.notify(
					"yamlls is not attached to this buffer (wait a moment after opening the file)",
					vim.log.levels.WARN
				)
				return
			end
		...
		local function insert_modeline(bufnr, schema_id)
			...
			if first_line:match("yaml%-language%-server:.*%$schema") then
				vim.notify(
					"Modeline already present at top of file; remove existing if you want to change it.",
					vim.log.levels.INFO
				)
				return
			end
		...
		local curated = {
			...
			{
				label = "Ansible (roles/tasks)",
				id = "https://json.schemastore.org/ansible-stable-2.9.json",
			},
		...
		local function open_full_schemastore_picker(bufnr)
			...
			vim.ui.select(items, {
				prompt = "SchemaStore: select schema",
				...
				vim.ui.select({ "No", "Yes - add modeline at top" }, { prompt = "Persist as modeline?" }, function(p)
					if p == "Yes - add modeline at top" then
						insert_modeline(bufnr, choice.id)
					end
				end)
			end)
		end
		...
		local function open_schema_picker(bufnr)
			...
			vim.ui.select(curated, {
				...
				vim.ui.select(
					{ "No", "Yes - add modeline" },
					{ prompt = "Persist chosen schema as modeline?" },
					function(p)
						if p == "Yes - add modeline" then
							insert_modeline(bufnr, choice.id)
						end
					end
				)
		...
		vim.defer_fn(function()
			...
			vim.keymap.set("n", "<leader>ys", function()
				open_schema_picker(bufnr)
			end, { buffer = bufnr, desc = "YAML: select schema" })
			vim.keymap.set("n", "<leader>yi", function()
				local client = get_yamlls_client(bufnr)
				if not client then
					vim.notify("yamlls not attached yet", vim.log.levels.INFO)
					return
				end
				local bufname = vim.api.nvim_buf_get_name(bufnr)
				local found = nil
